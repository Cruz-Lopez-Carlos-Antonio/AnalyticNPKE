#Analytical solution of the NPKE for feedback reactivities
...
f= open("Solutions_feedback_final.txt","w+")                   # Name of the output file
Beta_total = sum(Betas) 
for k in range(iteration):
    if k==0:
        rho =0                                                 # Reactivity equals to zero
    else:
        rho = rho+step*(a-b*n_0)                               # Reactivity approximation of the Feedback case.
    solucion = 0
    solution(L,Betas,Lambda_m,n_0,C_init,rho,step, vector_solution)
    neutron_density.append(vector_solution[-1][0])
    time_vector.append(k*step)
    ...
    f.write(cadena+"\n")
f.close()
peak = max(neutron_density)                             # Find the maximum value in the neutron density in the interval (0,Target)
time_peak = time_vector[neutron_density.index(peak)]    # Find the time related to the maximum value of the neutron density
print("Neutron peak:",peak)
print("Time of the peak:",time_peak)
print("Concentration of the precursors related to the neutron density peak")
print(vector_solution[neutron_density.index(peak)][1])
