...
As it was mentioned in **Section 5.3**, this code admits as inputs the decay constants and the $\beta$ values (as lists), as well as the reactivity $\rho$ and $\Lambda$. Two lists are generated by the code which contains the coefficients of $P(s)$ and its derivative.
```Python
...
This code generates the coefficients of the $H(s)$ polynomial that is given by Eq.(15). It requires the same input of the **Polyn_coeff_P**, but in addition it needs the initial conditions 
