...
$$C_k\left(t\right)=\frac{\beta_k}{\Lambda}\sum_{j=1}^{K+1}{\frac{n\left(0\right)Q\left(p_j\right)+H(p_j)}{P\prime(p_j)}\frac{\exp{\left(p_jt\right)}-\exp\funcapply(-\lambda_kt)}{p_j+\lambda_k}}+C_k\left(0\right)\exp\funcapply(-\lambda_kt) \tag{13}$$
